...
class BonusSelector(ui.Bar):
	...
	def Activate_rare(self):
		...
		self.Starter_rare_boni.SetText("6/7 Stop")
	...
	def __init__(self,sub_parent):
		...
		self.Starter.SetPosition(0,140)
		self.Starter.SetText("Schimba Start")
		...
		self.BlockText.SetText("Nu poti schimba pe acest item")
		self.BlockText.Show()
	...
	class ItemTab(ui.Bar):
		...
		def Switch_rare(self):
			...
			if lc == 0 and self.parentWindow.parentWindow.enable_rebuy.checked == 0:
				chat.AppendChat(2,"Slot %d: 6/7 Schimbare fara succes." % (self.index+1))
				self.bonusSelector.Deactivate_rare()
				return
			...
			if lc == 0:
				self.bonusSelector.Deactivate_rare()
				chat.AppendChat(2,"Slot %d: 6/7 Schimbare abandonata." % (self.index+1))
				return
		...
		def checkSwitch_rare(self):
			...
			if ok == 1 or self.prob_rare >= 90:
				if self.prob_rare >= 90:
					...
					chat.AppendChat(1,"Slot %d: 6/7 Gata de schimbare" % (self.index+1))
					return
		...
		def UpdateItem(self):
			...
			if vnum == 0 and self.vnum != 0:
				...
				if self.parentWindow.parentWindow.gameWindow != None:
					(w,h) = item.GetItemSize(1,2,3,4,5,6,7,8,9)
	...
	def AddTab_pre(self):
		if mouseModule.mouseController.isAttached():
			...
			if item.GetItemType() != 1 and item.GetItemType() != 2:
				...
				chat.AppendChat(2,"Nu poti schimba pe acest item")
				return
			
			for a in self.tabList:
				if self.tabList[a].index == attachedSlotPos:
					mouseModule.mouseController.DeattachObject()
					chat.AppendChat(2,"Acest slot este deja ocupat.")
					return
