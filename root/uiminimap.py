...
class MiniMap(ui.ScriptWindow):
	...
	def __init__(self):
		...
		if app.ENABLE_BATTLE_FIELD:
			self.exitQuestionDialog = None
			self.wndBattleField = uiBattleField.BattleFieldWindow()
			self.wndBattleField.BindMiniMap(self)
	...
	if app.ENABLE_BATTLE_FIELD:
		def OnAskExitQuestion(self):
			if self.exitQuestionDialog and self.exitQuestionDialog.IsShow():
				return
			
			exitQuestionDialog = uiCommon.QuestionDialog2()
			exitQuestionDialog.SetText1(uiScriptLocale.CURRENT_POINT % (player.GetTempBattlePoints()))
			exitQuestionDialog.SetText2(uiScriptLocale.EXIT_BATTLE_FIELD)
			exitQuestionDialog.SetAcceptEvent(lambda arg = True : self.ExitBattleField(arg))
			exitQuestionDialog.SetCancelEvent(lambda arg = False : self.ExitBattleField(arg))
			exitQuestionDialog.Open()
			self.exitQuestionDialog = exitQuestionDialog
			
		def OnCloseQuestionDialog(self):
			if self.exitQuestionDialog:
				self.exitQuestionDialog.Close()
	
			self.exitQuestionDialog = None
			
		def ExitBattleField(self, arg):
			if arg==True:
				self.exitQuestionDialog.Close()
				net.SendRequestExitBattle()
			else:
				self.exitQuestionDialog.Close()
		
		def OpenBattleField(self):
			if background.IsBattleFieldMap():
				self.OnAskExitQuestion()
				return
