...
class ItemToolTip(ToolTip):
	...
	def AddItemData(self, itemVnum, metinSlot, attrSlot = 0, flags = 0, window_type = player.INVENTORY, slotIndex = -1, transmutation = -1):
		...
		if self.check_sigillo(itemVnum) or itemVnum == 55002:
			if attrSlot[0][1] != 0:
				self.AppendSpace(5)
				self.AppendTextLine("|cFFFFFFFFSzint: |cFF1BFF00"+str(str(metinSlot[1]))+"("+str(str(metinSlot[2]))+")", self.SPECIAL_TITLE_COLOR)
				self.AppendTextLine("|cFFFFFFFFÉlet: |cFF1BFF00+"+pointop(str(attrSlot[0][1]))+"%", self.SPECIAL_POSITIVE_COLOR)
				self.AppendTextLine("|cFFFFFFFFVédelem: |cFF1BFF00+"+pointop(str(attrSlot[1][1]))+"%", self.SPECIAL_POSITIVE_COLOR)
				self.AppendTextLine("|cFFFFFFFFTámadás: |cFF1BFF00+"+pointop(str(attrSlot[2][1]))+"%", self.SPECIAL_POSITIVE_COLOR)
				self.AppendSpace(5)
				if metinSlot[1] >= 40:
					self.AppendTextLine("Extra bónusz:", self.SPECIAL_TITLE_COLOR)
					self.AppendTextLine("|cFF0df2f2+10% Szörny elleni erõt biztosít.", self.SPECIAL_POSITIVE_COLOR)
				if metinSlot[1] >= 60:
					self.AppendTextLine("|cFF0df2f2Készség károk +10%", self.SPECIAL_POSITIVE_COLOR)
				if metinSlot[1] >= 80:
					self.AppendTextLine("|cFF0df2f2+10% Félemberek elleni erõt biztosít.", self.SPECIAL_POSITIVE_COLOR)
				if metinSlot[1] >= 100:
					self.AppendTextLine("|cFF0df2f2+10% Képességekkel szembeni ellenállást biztosít.", self.SPECIAL_POSITIVE_COLOR)
				if metinSlot[1] < 40:
					self.AppendTextLine("Nincsen extra bónusza", self.DISABLE_COLOR)
				if itemVnum != 55002:
					days = (int(attrSlot[3][1])/60)/24
					hours = (int(attrSlot[3][1]) - (days*60*24)) / 60
					mins = int(attrSlot[3][1]) - (days*60*24) - (hours*60)
					self.AppendTextLine("|cFFFFFFFFMaradék idõ: %d Nap %d Óra %d Perc" % (days, hours, mins), self.NORMAL_COLOR)		
		...
		if self.sigilii_buffi(itemVnum):
			if attrSlot[0][1] != 0:     
				...
				self.AppendSpace(5)
				self.AppendTextLine("|cFFFFFFFFSzint: |cFF1BFF00"+str(str(attrSlot[0][1])), self.SPECIAL_TITLE_COLOR)
				self.AppendTextLine("|cFFFFFFFFInt: |cFF1BFF00"+str(attrSlot[1][1])+" ", self.SPECIAL_POSITIVE_COLOR)             #|cFFff5742
				self.AppendTextLine("|cFFFFFFFFBónusz: |cFF1BFF00"+str(attrSlot[2][1])+" ", self.SPECIAL_POSITIVE_COLOR)               #|cFF0df2f2
				self.AppendTextLine("|cFFFFFFFFExp: |cFF1BFF00"+str(str(metinSlot[0])), self.SPECIAL_POSITIVE_COLOR)
				self.AppendTextLine("|cFFFFFFFFSzüks. exp: |cFF1BFF00", self.SPECIAL_POSITIVE_COLOR)
				self.AppendSpace(5)
				
				if metinSlot[1] != 0:
					item.SelectItem(metinSlot[1])
					itemName = item.GetItemName()
					self.AppendTextLine("Felszerelt kosztüm:", self.SPECIAL_TITLE_COLOR)
					self.AppendTextLine("|cFF0df2f2"+str(str(itemName)), self.SPECIAL_TITLE_COLOR)
				elif metinSlot[1] == 0:
					self.AppendTextLine("Nincs felszerelve kosztüm.", self.DISABLE_COLOR)
				...
				if attrSlot[3][1] != 0:
					...
					self.AppendTextLine("Lejárt az idõ.", self.DISABLE_COLOR)
...
#Descriere_iteme
		if itemVnum in (71051, 71052, 91021, 91022, 91023, 91024, 91025, 91026, 91027, 91028, 91029):
			...
			serenity_list = {
				...
				91025 : ["Acest sigiliu confera samanului tau o durabilitate extra", "Durabilitate: + 1 nap", "Itemul dispare dupa utilizare!", self.SPECIAL_POSITIVE_COLOR],
				91026 : ["Acest sigiliu confera samanului tau o durabilitate extra", "Durabilitate: + 2 nap", "Itemul dispare dupa utilizare!", self.SPECIAL_POSITIVE_COLOR],
				91027 : ["Acest sigiliu confera samanului tau o durabilitate extra", "Durabilitate: + 7 nap", "Itemul dispare dupa utilizare!", self.SPECIAL_POSITIVE_COLOR],
				91028 : ["Acest sigiliu confera samanului tau o durabilitate extra", "Durabilitate: + 14 nap", "Itemul dispare dupa utilizare!", self.SPECIAL_POSITIVE_COLOR],
