...
class PetSystemMain(ui.ScriptWindow):
	...
	def __LoadWindow(self):
		...
		try:
			...
			self.itemexp.SetToggleDownEvent(lambda arg=0: self.extrabnspet(arg))
			self.itemexp.SetToggleUpEvent(lambda arg=1: self.extrabnspet(arg))
	...
	def extrabnspet(self, mode):
		if mode == 0:
			net.SendChatPacket("/petextrabns")
			self.itemexp.Enable()
	...
	def SetExperience(self, expm, expi, exptot):
		...
		#####
		self.tooltipexp[0].SetText("|cFFFFFFFFTapasztalat: |cFF1BFF00%.2f%%" % (expmp))
		self.tooltipexp[1].SetText("|cFFFFFFFFTárgy exp: |cFF1BFF00%.2f%%" % expip)
